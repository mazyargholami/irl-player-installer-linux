...
sudo bash tests/run-e2e.sh        # full test suite (56 checks), must run as root
```

The e2e suite runs the real scripts inside a fake root under `/tmp` (system paths are sed-redirected, `systemctl`/`apt-get`/`dpkg`/`useradd` are PATH-stubbed, a local `python3 -m http.server` plays GitHub Pages). It makes no changes to the host. It covers: fresh install, all `irl-update` paths (no-op, changed script, captive portal, flock), deleted-service propagation, a real-time freeze-watchdog simulation (takes ~30 s), and uninstall. **Extend it whenever you add a service or change updater/watchdog logic.**
...
3. **Self-healing**: `irl-watchdog` hashes a `grim` screenshot every 30 s; a pixel-identical screen for 5 min = frozen → restart the kiosk service (max 3, counter clears only after 5 min of continuously changing screen) → reboot (max once per 2 h, timestamp in `/var/lib/irl-player/`). It deliberately does nothing when the screen can't be captured or the kiosk is intentionally stopped. `/etc/systemd/system.conf.d/irl-watchdog.conf` arms the Pi's hardware watchdog for full-OS hangs.
...
3. Single commit, PR, merge to `main`. Devices converge within the hour; the site footer confirms what's deployed.
