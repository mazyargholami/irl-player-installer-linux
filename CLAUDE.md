...
6. **IRL Gateway** (`/opt/irl-gateway/`, service `irl-gateway`): bridges a USB-attached ESP32 master board (and its worker fleet) to MQTT. `gateway.py` is embedded in `install.sh` (source of truth: the `irl-microcontroller` repo) with a venv (`pyserial`, `paho-mqtt>=2.0` — newer than apt's). Its MQTT config is **fetched, never shipped**: `ExecStartPre` runs `irl-gateway-config`, which GETs `https://config.theirlnetwork.com/mqtt-config?serial=<hw serial>` (a Cloudflare Worker owned by the user; `MQTT_JSON` + `ALLOWLIST` variables in their dashboard — see README "Cloudflare Worker config service" for the operator guide, including the "variables need Edit code → Deploy" gotcha). Approved devices get the config (root-only `mqtt.json`, kept as an offline cache); unapproved/no-cache devices retry ~1/min — that retry IS the approval request, visible in the Worker's Observability logs as `serial=<x> refused`. `RuntimeMaxSec=1d` restarts the gateway daily so rotations/approvals propagate within a day. Devices without an ESP32 attached idle harmlessly.
