...
# /opt/irl-gateway includes the per-device mqtt.json credentials and the
# panel-delivered broker CA (config-ca.pem) — uninstall means "back to
# normal", so both go too
